package com.example.prof.util

import com.example.prof.model.AppState
import com.example.prof.model.DataModel
import com.example.prof.model.Meanings
import com.example.prof.model.Translation
import com.example.prof.model.dao.HistoryEntity
...
fun mapHistoryEntityToSearchResult(list: List<HistoryEntity>): List<DataModel> {
    ...
    if (!list.isNullOrEmpty()) {
        for (entity in list) {
            searchResult.add(DataModel(entity.word, listOf(Meanings(Translation( entity.description),entity.url))))
        }
    }
    return searchResult
}
    ...
    return when (appState) {
        is AppState.Success -> {
            ...
            if (searchResult.isNullOrEmpty() || searchResult[0].text.isNullOrEmpty()) {
                ...
                HistoryEntity(searchResult[0].text!!, searchResult[0].meanings?.get(0)?.translation.toString(),searchResult[0].meanings?.get(0)?.imageUrl)
            }
        }
        else -> null
    }
