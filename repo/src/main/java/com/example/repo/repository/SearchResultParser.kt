...
import com.example.repo.repository.dao.HistoryEntity
    ...
    if (!list.isNullOrEmpty()) {
        for (entity in list) {
            searchResult.add(
                DataModel(
                    entity.word,
                    listOf(Meanings(Translation(entity.description), entity.url))
                )
            )
        }
    }
    ...
    return when (appState) {
        is AppState.Success -> {
            ...
            if (searchResult.isNullOrEmpty() || searchResult[0].text.isNullOrEmpty()) {
                ...
                HistoryEntity(
                    searchResult[0].text!!,
                    searchResult[0].meanings?.get(0)?.translation.toString(),
